  ...
  Future<List<Recipe>> _searchRecipes(String query) async {
    try {
      ...
      return all.where((r) {
        ...
        final categoryMatch = r.tags.any(
  ...
  Future<List<Recipe>> _getStockBasedRecommendations() async {
    try {
      ...
      final availableIngredients = stockSnap.docs
          .map(
            (d) =>
                ((d.data()['itemName'] as String?) ?? '').toLowerCase().trim(),
          )
      ...
      // Fetch user's favorite recipe IDs to boost recommendation score
      final favDocs = await FirebaseFirestore.instance
          .collection('users')
          .doc(uid)
          .collection('favorites')
          .get();
      final favIds = favDocs.docs.map((d) => d.id).toSet();

      // Score each recipe by stock matches + favorites
      final scored = <MapEntry<Recipe, int>>[];
      for (final recipe in recipes) {
        int matchCount = recipe.ingredients
            .where(
              (ing) => availableIngredients.any(
                (stock) =>
                    ing.toLowerCase().contains(stock) ||
                    stock.contains(ing.toLowerCase()),
              ),
            )
            .length;

        // Boost the score if it's a user favorite
